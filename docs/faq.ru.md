...
[[fcitx5-android#175](https://github.com/fcitx 5-android/fcitx5-android/pull/175)добавлена поддержка импорта метода ввода на основе таблицы, поэтому после `0.0.3-43` вы можете импортировать таблицу, выполнив следующие действия:

1. Получите файл конфигурации метода ввода (расширение имени файла должно быть `.conf` или `.conf.in`) и табличный словарь (расширение имени файла должно быть `.dict` или `.txt`) для импорта. Возьмем, к примеру, "zhengma", вы можете скачать `zhengma.conf.in` и `zhengma.txt` по адресу [fcitx/fcitx5-table-extra](https://github.com/fcitx/fcitx5-table-extra).
2. Откройте приложение, перейдите в раздел Дополнения → Таблица (нажмите на шестеренку справа) → Управление методами ввода в таблицу и нажмите кнопку "Плюс" в правом нижнем углу. В появившемся диалоговом окне выберите второй пункт, т.е. "Из отдельных файлов".
3. В новом диалоговом окне нажмите "Файл конфигурации метода ввода" и выберите `zhengma.conf.in`, полученный на шаге 1; нажмите "Словарь таблиц" и выберите `zhengma.txt`, полученный на шаге 1; и нажмите кнопку "ОК".
...
Чтобы импортировать таблицу вручную, вам необходимо иметь правильный файл конфигурации метода ввода `.conf` и преобразовать словарь таблиц в формате `.txt` в формат `.dict` с помощью инструмента командной строки `libime_tabledict`.

1. Получите файл конфигурации метода ввода (расширение имени файла должно быть `.conf` или `.conf.in`) и табличный словарь (расширение имени файла должно быть `.dict` или `.txt`) для импорта. Возьмем, к примеру, "zhengma", вы можете скачать `zhengma.conf.in` и `zhengma.txt` по адресу [fcitx/fcitx5-table-extra](https://github.com/fcitx/fcitx5-table-extra).
2. Откройте `zhengma.conf.in` в вашем любимом редакторе и измените путь, указанный после `[Table] File=` на `table/zhengma.dict`.
3. Запустите команду `libya_tabledict zhengma.txt zhengma.dict`, чтобы преобразовать табличный словарь. Этот инструмент обычно предоставляется пакетом под названием `libya` или `libya-bin`  в дистрибутивах Linux.
...
[[fcitx5-android#170](https://github.com/fcitx 5-android/fcitx5-android/pull/170) добавляет удаление множественного выбора, поэтому после `0.0.3-37`
вы можете нажать кнопку редактирования (карандашом) в правом верхнем углу списка методов ввода, чтобы перейти к множественному выбору. режим,
в котором флажок появится перед названием метода ввода. Вы можете выбрать методы ввода, которые необходимо удалить,
и нажать кнопку удалить (корзина для мусора) в правом верхнем углу.
...
Чтобы очистить предварительное редактирование и кандидатов, нажмите и удерживайте клавишу Backspace, проведя пальцем влево на некоторое расстояние.
...
!!! внимание
    ...
    Вы можете получить доступ к файлам в `/sdcard/Android/data/org.fcitx.fcitx5.android/files/`, выбрав "Fcitx 5 для Android" на боковой панели DocumentsUI. Разрешения стороннего файлового менеджера /adb/root не требуются.
